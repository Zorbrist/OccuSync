...
import { ArrowRight, Briefcase, CheckCircle, CreditCard, Search, Users, AlertCircle } from "lucide-react";
...
const TOOLTIP_LABEL_STYLE = { color: "#64748B", marginBottom: "6px", fontWeight: 600 };
const LEGEND_STYLE = { color: "#475569", fontSize: "13px", fontWeight: 600, paddingTop: "12px" };
    ...
    return (
      <div className="flex min-h-[calc(100vh-80px)] items-center justify-center bg-[#E8EDF2]">
        <div className="bg-white p-8 rounded-[2rem] shadow-xl text-center max-w-sm">
          <h3 className="text-[#0F172A] text-xl font-bold mb-2">Failed to load</h3>
          <p className="text-slate-600 text-sm mb-6 font-medium">{error || "Data unavailable"}</p>
          <button onClick={() => window.location.reload()} className="px-6 py-3 bg-violet-600 text-white rounded-full text-sm font-bold w-full shadow-md">
            Retry Connection
          </button>
        </div>
      </div>
      ...
      <div className="relative z-10 max-w-[1500px] mx-auto px-6 lg:px-12 space-y-8 pt-4">
        ...
        <BlurFade delay={0.1}>
          <div className="flex items-center justify-between py-2 border-b border-slate-200/50 pb-4">
            <div>
              <h2 className="text-3xl font-bold tracking-tight text-[#0F172A]">Platform Overview</h2>
              <p className="text-sm font-semibold text-slate-500 mt-1.5 uppercase tracking-wider">Admin control center and analytics</p>
            </div>
          </div>
        </BlurFade>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              >
                <div className="flex justify-between items-start mb-6">
                  <div className="w-12 h-12 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-600"><Users size={20} /></div>
                  <span className="text-[11px] font-bold uppercase tracking-widest text-slate-400">Total Users</span>
                </div>
                <div>
                  <h3 className="text-4xl font-black text-[#0F172A]">{data.user_overview.total_users}</h3>
                  <p className="text-xs font-semibold text-slate-500 mt-1.5">Clients & Businesses</p>
                </div>
              </div>
              ...
              >
                <div className="flex justify-between items-start mb-6">
                  <div className="w-12 h-12 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-600"><Briefcase size={20} /></div>
                  <span className="text-[11px] font-bold uppercase tracking-widest text-slate-400">Services</span>
                </div>
                <div>
                  <h3 className="text-4xl font-black text-[#0F172A]">{data.service_job_overview.total_services}</h3>
                  <p className="text-xs font-semibold text-slate-500 mt-1.5">Active directory listings</p>
                </div>
              </div>
              ...
              >
                <div className="flex justify-between items-start mb-6">
                  <div className="w-12 h-12 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center text-emerald-600"><CheckCircle size={20} /></div>
                  <span className="text-[11px] font-bold uppercase tracking-widest text-slate-400">Completed</span>
                </div>
                <div>
                  <h3 className="text-4xl font-black text-[#0F172A]">{data.service_job_overview.jobs_completed}</h3>
                  <p className="text-xs font-semibold text-slate-500 mt-1.5">Jobs successfully fulfilled</p>
                </div>
              </div>
              ...
              >
                <div className="flex justify-between items-start mb-6">
                  <div className="w-12 h-12 rounded-full bg-violet-50 border border-violet-100 flex items-center justify-center text-violet-600"><CreditCard size={20} /></div>
                  <span className="text-[11px] font-bold uppercase tracking-widest text-violet-400">Revenue Volume</span>
                </div>
                <div>
                  ...
                  <p className="text-xs font-semibold text-slate-500 mt-1.5">Total platform transactions</p>
                </div>
              </div>
        ...
        <BlurFade delay={0.3}>
          <div className="bg-[#F1F5F9] rounded-[2.5rem] p-8 shadow-[inset_0_2px_15px_rgba(255,255,255,1)] border border-white/60">
                ...
                <h3 className="text-xl font-bold text-[#0F172A] flex items-center gap-3">
                  ...
                  {data.pending_businesses.length > 0 && (
                    <span className="px-3.5 py-1.5 bg-amber-100 text-amber-700 text-[11px] font-black rounded-full uppercase tracking-wider flex items-center gap-1.5">
                      <AlertCircle size={14} /> Action Needed ({data.pending_businesses.length})
                    </span>
                  )}
                </h3>
              </div>
            ...
            <div className={`overflow-x-auto overflow-y-auto max-h-[400px] bg-white rounded-[1.5rem] shadow-[0_8px_30px_rgba(0,0,0,0.03)] border border-slate-100 ${scrollbarClasses}`}>
              <table className="w-full text-left min-w-[800px]">
                <thead className="sticky top-0 z-10">
                  ...
                  <tr className="text-xs font-bold text-slate-400 uppercase tracking-widest border-b border-slate-200">
                    <th className="py-5 pl-8 bg-white rounded-tl-[1.5rem]">Business Entity</th>
                    <th className="py-5 bg-white">Industry</th>
                    <th className="py-5 bg-white">Contact Details</th>
                    <th className="py-5 text-right pr-8 bg-white rounded-tr-[1.5rem]">Action</th>
                  </tr>
                </thead>
                <tbody className="text-sm">
                  {data.pending_businesses.map((business) => (
                    <tr key={business.id} onClick={() => setSelectedBusiness(business)} className="hover:bg-slate-50 transition-colors border-b border-slate-100 cursor-pointer group">
                      <td className="py-5 pl-8">
                        <p className="font-bold text-[#0F172A] group-hover:text-violet-700 transition-colors">{business.name}</p>
                        <p className="text-xs font-semibold text-slate-500 mt-1">Reg No: {business.registration_no || "N/A"}</p>
                      </td>
                      <td className="py-5 text-slate-600 font-semibold">{business.industry}</td>
                      <td className="py-5 text-slate-500 font-medium">{business.email}</td>
                  ...
                  {data.pending_businesses.length === 0 && (
                    <tr><td colSpan={4} className="py-12 text-center text-slate-500 font-semibold bg-slate-50">The registration queue is currently clear.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
          ...
          <BlurFade delay={0.4} className="h-full">
            <div className="bg-[#F1F5F9] rounded-[2.5rem] p-8 shadow-[inset_0_2px_15px_rgba(255,255,255,1)] border border-white/60 h-full flex flex-col">
              <h3 className="text-xl font-bold text-[#0F172A] mb-8">Growth Analytics</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1">
                ...
                <div className="bg-white rounded-[1.5rem] p-6 shadow-[0_8px_30px_rgba(0,0,0,0.03)] border border-slate-100 min-h-[250px] flex flex-col">
                  <h4 className="text-[11px] font-black uppercase tracking-widest text-slate-400 mb-6">Registrations (30 Days)</h4>
                  <div className="h-[200px] flex-1">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={registrationData}>
                        <CartesianGrid strokeDasharray="3 3" stroke={GRID_STROKE} vertical={false} />
                        <XAxis dataKey="date" tick={AXIS_TICK} axisLine={false} tickLine={false} dy={10} />
                        <YAxis allowDecimals={false} tick={AXIS_TICK} axisLine={false} tickLine={false} dx={-10} width={30} />
                        <RechartsTooltip contentStyle={TOOLTIP_STYLE} labelStyle={TOOLTIP_LABEL_STYLE} cursor={{ stroke: '#E2E8F0', strokeWidth: 2 }} />
                        <Line type="monotone" dataKey="registrations" stroke="#7C3AED" strokeWidth={4} dot={false} activeDot={{ r: 7, fill: "#7C3AED", stroke: "#FFFFFF", strokeWidth: 3 }} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-[1.5rem] p-6 shadow-[0_8px_30px_rgba(0,0,0,0.03)] border border-slate-100 min-h-[250px] flex flex-col">
                  <h4 className="text-[11px] font-black uppercase tracking-widest text-slate-400 mb-6">User Distribution</h4>
                  <div className="h-[200px] flex-1">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={userChartData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={4}>
                          {userChartData.map((_, index) => (
                            <Cell key={`cell-${index}`} fill={CHART_COLORS[index % CHART_COLORS.length]} stroke="none" />
                          ))}
                        </Pie>
                        <RechartsTooltip contentStyle={TOOLTIP_STYLE} itemStyle={{ color: '#0F172A', fontWeight: 700 }} />
          ...
          <BlurFade delay={0.5} className="h-full">
            <div className="bg-[#F1F5F9] rounded-[2.5rem] p-8 shadow-[inset_0_2px_15px_rgba(255,255,255,1)] border border-white/60 h-full flex flex-col">
              <div className="flex items-center justify-between mb-8">
                <h3 className="text-xl font-bold text-[#0F172A]">Live Job Feed</h3>
                <button 
                  onClick={() => navigate("/admin/jobs")}
                ...
                <table className="w-full text-left min-w-[500px]">
                  <thead className="sticky top-0 z-10">
                    ...
                    <tr className="text-[11px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-200">
                      <th className="py-4 pl-6 bg-white rounded-tl-[1.5rem]">Service Required</th>
                      <th className="py-4 bg-white">Scheduled For</th>
                      <th className="py-4 text-right pr-6 bg-white rounded-tr-[1.5rem]">Current Status</th>
                    </tr>
                  </thead>
                  <tbody className="text-sm">
                    {data.latest_jobs.map((job) => (
                      <tr key={job.id} className="hover:bg-slate-50 transition-colors border-b border-slate-100">
                        <td className="py-4 pl-6">
                          <p className="font-bold text-[#0F172A]">{job.service_name}</p>
                          <p className="text-xs font-semibold text-slate-500 mt-1">{job.business_name}</p>
                        </td>
                        <td className="py-4 text-slate-600 font-semibold">
                          {job.date ? new Date(job.date).toLocaleDateString("en-MY", { day: 'numeric', month: 'short' }) : "Pending Date"}
                        </td>
                    ...
                    {data.latest_jobs.length === 0 && (
                      <tr><td colSpan={3} className="py-12 text-center text-slate-500 font-semibold bg-slate-50 rounded-xl">No active jobs in the feed.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
